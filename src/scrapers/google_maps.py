...
результатов без капчи с текущей (не-российской) сети. (Здесь стояло "в отличие
от 2ГИС" — с 29.08.2026 неактуально: 2ГИС тоже отдаёт данные с этой сети, его
"блок по IP" оказался блоком по TLS-отпечатку клиента, см. gis2.py.)
